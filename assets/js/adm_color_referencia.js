...
function getProductsList(desde = 0, hasta = 100){
    ...
    $.ajax({
        type: "POST",
        url: "../controller/colorreferencia/getColorReferenciaList.php",
        ...
        success: function (info) {
            ...
            if(mi_json.estado == 1 ){
                ...
                mi_json.lista.forEach(mi_sku => {
                    tabla_body.innerHTML += `<tr id="pro_identity_` + mi_sku.IDcolor_referencia + `"  ` + ( mi_sku.estado == 0 ? ` style="color:gray;" ` : ``) + `  >
                    ...
                    <td class="p-1"><span style="width: 20px; height: 12px; display:inline-block; margin-right:20px; background:` + (mi_sku.hexadecimal != null ? mi_sku.hexadecimal : `#000000`) + `;"></span>` + (mi_sku.hexadecimal != null ? mi_sku.hexadecimal : `#000000`) + `</td>
                </tr>`;
...
document.getElementById("btn_producto_exportar").onclick = function(event){
    console.log("e");
    //$("#mi_modal_importar_productos").modal('show');
    window.location.href = "../controller/colorreferencia/exportarColorReferencia.php";
};
...
function onProductDelete(objeto, value){
    let mensaje_modal = "";
    if(value==1){
        mensaje_modal ="¿Deseas activar el color?";
    }else{
        mensaje_modal ="¿Deseas desactivar el color?";
    }
    ...
    alertify.confirm(mensaje_modal, function (param1) {
        ...
        $.ajax({
            type: "POST",
            url: "../controller/colorreferencia/desactivarColorReferencia.php",
...
function onProductEditable(objeto){
    ...
    //$("#pro_color_hex").val($(fila[2]).text());

    $("#pro_color_hex").spectrum("set", $(fila[2]).text());
...
function onResetFormulario(){
    ...
    //$('#pro_color_hex').val("#000000");
    $("#pro_color_hex").spectrum("set", "#000000");
...
function saverOrUpdateProduct(parametros){
    $.ajax({
        type: "POST",
        url: "../controller/colorreferencia/saveOrUpdateColorReferencia.php",
        data: parametros,
        beforeSend: function () {
        },
        success: function (info) {
