...
computed:{
  ...
  /*totalVentas(){
    this.ventas.forEach(
      venta => { if (!this.totalVentas.find(ven => ven.n_orden == venta.n_orden && ven.email == venta.email)) {
          const { n_orden,nombres,apellidos,email,direccion_entrega,referencia_entrega} = venta;
          this.totalVentas.push({ n_orden,nombres,apellidos,email,direccion_entrega,referencia_entrega });
      }
    });
