            ...
            if(mi_json.estado == 1 ){
                ...
                mi_json.lista.forEach(element => {
                    ...
                    <tr>
                        ...
                        <td class="p-1">` + element.codigoERP + `</td>
...
document.getElementById("export-stock").onclick = function(event){
    // getProductStockExportList();
    // let totalRowCount = $("#tbody_cargar tr").length;
    // if(totalRowCount<=1){
...
// function exportExcelData(exportcabecera,exportdata){
//     let cabecera = JSON.stringify(exportcabecera); 
//     let  data = JSON.stringify(exportdata);
//     window.location.href = "../controller/export_data.php?cabecera="+cabecera+"&data="+data;
    /*$.ajax({
        type: "POST",
        url: "../controller/export_data.php",
        data: {  cabecera: exportcabecera , data : exportdata },
        // dataType: "json",
        beforeSend: function () {
            // alertify.success('Exportando datos...');
        },
        success: function () {
            // console.log(result);
            alertify.success('Exportado');
            miOpenAndClosedContent("my_capa_negra_tabla");
        },
        error: function () {
...
// function getProductStockExportList(){
//     $.ajax({
//         type: "POST",
//         url: "../controller/stock/getStockList.php",
//         data: {desde: 0, hasta: 0,subcategoria: filter_subcategoria, nombre: filter_nombre, codigoERP: filter_codigoERP, almacen:filter_almacen, setLimit:0 },
//         beforeSend: function () {
//             miOpenAndClosedContent("my_capa_negra_tabla");
//         },
//         success: function (info) {
...
//             if(result.estado == 1 ){
//                 // let cabecera = ["Producto","SKU","Almacen","ID tallero","ID tipo","Talla","Stock Total","Stock Reservado (NC)","Stock Disponible","Fecha Actualizacion"];
//                 // exportExcelData(cabecera,result.lista);
...
function selectFactoryList(componente = "", url = "", paramametro = 100){
    ...
    $.ajax({
        ...
        success: function (info) {
            // mi_select.innerHTML = `<option value="0">[Ninguno]</option>`;
            mi_select.innerHTML = ``;
            let mi_json = JSON.parse(info);
            if(mi_json.estado == 1 ){
