...
function getProductsList(desde = 0, hasta = 100){
    ...
    $.ajax({
        type: "POST",
        url: "../controller/color/getColorList.php",
        ...
        success: function (info) {
            ...
            if(mi_json.estado == 1 ){
                ...
                mi_json.lista.forEach(mi_sku => {
                    tabla_body.innerHTML += `<tr id="pro_identity_` + mi_sku.IDcolor + `"  ` + ( mi_sku.estado == 0 ? ` style="color:gray;" ` : ``) + `  >
...
document.getElementById("btn_producto_exportar").onclick = function(event){
    console.log("e");
    //$("#mi_modal_importar_productos").modal('show');
    window.location.href = "../controller/color/exportarColor.php";
};
...
function onProductDelete(objeto, value){
    ...
    alertify.confirm(mensaje_modal, function (param1) {
        ...
        $.ajax({
            type: "POST",
            url: "../controller/color/desactivarColor.php",
...
function saverOrUpdateProduct(parametros){
    $.ajax({
        type: "POST",
        url: "../controller/color/saveOrUpdateColor.php",
        data: parametros,
        beforeSend: function () {
        },
        success: function (info) {
