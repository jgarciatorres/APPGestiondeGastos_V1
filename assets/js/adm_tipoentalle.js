...
function getProductsList(desde = 0, hasta = 100){
    ...
    $.ajax({
        type: "POST",
        url: "../controller/tipoentalle/getTipoEntalleList.php",
        ...
        success: function (info) {
            ...
            if(mi_json.estado == 1 ){
                ...
                mi_json.lista.forEach(mi_sku => {
                    tabla_body.innerHTML += `<tr id="pro_identity_` + mi_sku.IDentalle + `"  ` + ( mi_sku.estado == 0 ? ` style="color:gray;" ` : ``) + `  >
...
document.getElementById("btn_producto_exportar").onclick = function(event){
    console.log("e");
    //$("#mi_modal_importar_productos").modal('show');
    window.location.href = "../controller/tipoentalle/exportarTipoEntalle.php";
};
...
function onProductDelete(objeto, value){
    let mensaje_modal = "";
    if(value==1){
        mensaje_modal ="¿Deseas activar el tipo de entalle?";
    }else{
        mensaje_modal ="¿Deseas desactivar el tipo de entalle?";
    }
    ...
    alertify.confirm(mensaje_modal, function (param1) {
        ...
        $.ajax({
            type: "POST",
            url: "../controller/tipoentalle/desactivarTipoEntalle.php",
...
function saverOrUpdateProduct(parametros){
    $.ajax({
        type: "POST",
        url: "../controller/tipoentalle/saveOrUpdateTipoEntalle.php",
        data: parametros,
        beforeSend: function () {
        },
        success: function (info) {
