...
function getProductsList(desde = 0, hasta = 100){
    ...
    $.ajax({
        type: "POST",
        url: "../controller/tipobota/getTipoBotaList.php",
        ...
        success: function (info) {
            ...
            if(mi_json.estado == 1 ){
                ...
                mi_json.lista.forEach(mi_sku => {
                    tabla_body.innerHTML += `<tr id="pro_identity_` + mi_sku.IDbota + `"  ` + ( mi_sku.estado == 0 ? ` style="color:gray;" ` : ``) + `  >
...
document.getElementById("btn_producto_exportar").onclick = function(event){
    console.log("e");
    //$("#mi_modal_importar_productos").modal('show');
    window.location.href = "../controller/tipobota/exportarTipoBota.php";
};
...
function onProductDelete(objeto, value){
    let mensaje_modal = "";
    if(value==1){
        mensaje_modal ="¿Deseas activar el tipo de bota?";
    }else{
        mensaje_modal ="¿Deseas desactivar el tipo de bota?";
    }
    ...
    alertify.confirm(mensaje_modal, function (param1) {
        ...
        $.ajax({
            type: "POST",
            url: "../controller/tipobota/desactivarTipoBota.php",
...
function saverOrUpdateProduct(parametros){
    $.ajax({
        type: "POST",
        url: "../controller/tipobota/saveOrUpdateTipoBota.php",
        data: parametros,
        beforeSend: function () {
        },
        success: function (info) {
